...
[task_local]
#京喜财富岛提现
0 0 * * * https://gitee.com/misyi/jd-own/raw/master/jd_cfdtx.js, tag=京喜财富岛提现, img-url=https://raw.githubusercontent.com/Orz-3/mini/master/Color/jd.png, enabled=true
 */
...
function cashOut() {
    return new Promise(async (resolve) => {
        $.get(
            taskUrl(
                `consume/CashOut`,
                `ddwMoney=100&dwIsCreateToken=0&ddwMinPaperMoney=150000&strPgtimestamp=${token['timestamp']}&strPhoneID=${token['phoneid']}&strPgUUNum=${token['farm_jstoken']}`
            ),
            async (err, resp, data) => {
                try {
                    $.log(data);
