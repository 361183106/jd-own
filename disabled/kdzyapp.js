...
注册地址：http://kdzhy.mlyougame.com:82/web/page/qr.html?c=110234
不含邀请链接
需要实名认证后绑定支付宝
kdzyPhone:手机号#密码
export kdzyPhone='手机号#密码'
boxjs地址:https://raw.fastgit.org/byxiaopeng/myscripts/main/byxiaopeng.boxjs.json
*/

// new Env('口袋庄园');
// cron: 9 11 * * *
const $ = new Env('口袋庄园APP签到');
const notify = $.isNode() ? require('./sendNotify') : '';
let status;
status = (status = ($.getval("kdzystatus") || "1")) > 1 ? `${status}` : ""; // 账号扩展字符
let kdzyPhoneArr = [];
...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    if (!$.isNode()) {
      kdzyPhoneArr.push($.getdata('kdzyPhone'))
      let kdzycount = ($.getval('kdzycount') || '1');
      for (let i = 2; i <= kdzycount; i++) {
        kdzyPhoneArr.push($.getdata(`kdzyPhone${i}`))
      }
      console.log(`-------------共${kdzyPhoneArr.length}个账号-------------\n`)
      ...
      Object.keys(kdzyPhones).forEach((item) => {
        if (kdzyPhones[item]) {
...
function login(timeout = 0) {
  return new Promise((resolve) => {
    let url = {
      ...
      body: `appKey=5ded368547b86b33&mobile=${user}&password=${mima}`,
    }
...
function sign(timeout = 0) {
  return new Promise((resolve) => {
    let url = {
      url: `${host}/home/user/everyDaySignIn`,
      headers: {
