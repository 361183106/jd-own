...
目前10R还是秒到的   ，进入软件有点卡  耐心等金币出现吧

食用方法：打开软件提示获取气泡body，然后点击金币看一个视频获取视频body
...
!(async () => {
  ...
  for (let i = 2; i <= wzccycount; i++) {
    ...
    //console.log(`=================== 共${wzccyhdArr.length}个账号 ==================\n`)
      for (let i = 0; i < wzccyhdArr.length; i++) {
        if (wzccyhdArr[i]) {
         
          wzccyurl = wzccyurlArr[i];
          wzccyhd = wzccyhdArr[i];
          wzccybody = wzccybodyArr[i];
          wzccy2body = wzccy2bodyArr[i];
          $.index = i + 1;
          console.log(`\n【 王者猜成语 账号${$.index} 】`)
...
function qpid(timeout = 0) {
          return new Promise((resolve) => {
                 let url = {
                           url : `http://api.pp.tongdongsu.com/idiom-amu-api/bubble/loadBubble.do`,
                           headers : JSON.parse(wzccyhd),
                           body : wzccybody,
                          }
                   $.post(url, async (err, resp, data) => {
                           try {
                               ...
                               if(data.code == 0){
                                ...
                                id = data.data.bubbleBeans[RT(0,4)].id
                                //console.log(id)
...
function lqp(timeout = 0) {
  return new Promise((resolve) => {
    body = (wzccy2body).replace(/id=\d+&/g,`id=${id}&`)
let url = {
        url : `http://api.pp.tongdongsu.com/idiom-amu-api/bubble/receiveBubble.do`,
        headers : JSON.parse(wzccyhd),
        body : body,
}
      $.post(url, async (err, resp, data) => {
        ...
        $.log(`\n【领取金币】：${data.data.golds}\n【现有金币】：${data.data.userAllGolds}`)

       $.message += `【领取金币】：${data.data.golds}\n【现有金币】：${data.data.userAllGolds}\n`
