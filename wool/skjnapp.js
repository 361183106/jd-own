...
注册地址：https://yijianbwgd.com/h6/#/pages/reg/reg
skjnPhone:手机号#密码
export skjnPhone='手机号#密码'
boxjs地址:https://cdn.jsdelivr.net/gh/byxiaopeng/myscripts@main/byxiaopeng.boxjs.json
*/
// new Env('时空聚能');
// cron: 10 8 * * *
const $ = new Env('时空聚能APP签到');
const notify = $.isNode() ? require('./sendNotify') : '';
let status;
status = (status = ($.getval("skjnstatus") || "1")) > 1 ? `${status}` : ""; // 账号扩展字符
let skjnPhoneArr = [];
let skjnPhone = $.isNode() ? (process.env.skjnPhone ? process.env.skjnPhone : "") : ($.getdata('skjnPhone') ? $.getdata('skjnPhone') : "");
let skjnPhones = "";
let DD = RT(10000, 15000);
let tz = ($.getval('skjntz') || '1');
let host=`https://yijianbwgd.com`;
GXSJ = '【更新时间】： 2021/11/22更新';
$.message = ''
...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    if (!$.isNode()) {
      skjnPhoneArr.push($.getdata('skjnPhone'))
      let count = ($.getval('skjncount') || '1');
      for (let i = 2; i <= count; i++) {
        skjnPhoneArr.push($.getdata(`skjnPhone${i}`))
      }
      if (!skjnPhoneArr[0]) {
        $.log(`\n【傻吊提示】：你没有填写账户密码跑个嘚`)
        $.message += `\n【傻吊提示】：你没有填写账户密码跑个嘚`
        ...
        console.log(`-------------共${skjnPhoneArr.length}个账号-------------\n`)
      }
      for (let i = 0; i < skjnPhoneArr.length; i++) {
        if (skjnPhoneArr[i]) {
          skjnPhone = skjnPhoneArr[i];
          $.index = i + 1;
          console.log(`\n开始【时空聚能账户 ${$.index}】`)
          zhanghu = skjnPhone.split('#')
          user = zhanghu[0]
          mima = zhanghu[1]
          await byxiaopeng()
        }
      ...
      if (process.env.skjnPhone && process.env.skjnPhone.indexOf('@') > -1) {
        skjnPhoneArr = process.env.skjnPhone.split('@');
        console.log(`您选择的是用"@"隔开\n`)
      } else {
        skjnPhones = [process.env.skjnPhone]
      };
      Object.keys(skjnPhones).forEach((item) => {
        if (skjnPhones[item]) {
          skjnPhoneArr.push(skjnPhones[item])
        }
      })
      if (!skjnPhoneArr[0]) {
        $.log(`\n【傻吊提示】：你没有填写账户密码跑个嘚`)
      ...
      for (let k = 0; k < skjnPhoneArr.length; k++) {
        skjnPhone = skjnPhoneArr[k]
        $.index = k + 1;
        console.log(`\n开始【时空聚能账户 ${$.index}】`)
        zhanghu = skjnPhone.split('#')
        user = zhanghu[0]
