...
const $ = new Env('炎兔');
let status;
status = (status = ($.getval("ytstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
const  ythdArr = [],ytbodyArr = [],ytcount = ''
let ythd = process.env.ythd
let ytbody = process.env.ytbody
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        let ytcount = ($.getval('ytcount') || '1');
        for (let i = 2; i <= ytcount; i++) {

            ythdArr.push($.getdata(`ythd${i}`))
            ytbodyArr.push($.getdata(`ytbody${i}`))
        }
        console.log(
        ...
        console.log(`=================== 共${ythdArr.length}个账号 ==================\n`)
        for (let i = 0; i < ythdArr.length; i++) {
            if (ythdArr[i]) {
...
function ytck() {
    if ($request.url.indexOf("signin") > -1) {
        ...
        const ythd = JSON.stringify($request.headers)
        if(ythd)    $.setdata(ythd,`ythd${status}`)
        $.log(ythd)

        const ytbody = $request.body
        if(ytbody)     $.setdata(ytbody,`ytbody${status}`)
        $.log(ytbody)

        $.msg($.name,"",`炎兔${status}headers获取成功`)
...
//签到
function ytqd(timeout = 0) {
    return new Promise((resolve) => {
        token = ytbody.match(/token=(\w+-\w+-\w+-\w+-\w+)&/)[1]
        sign =MD5_Encrypt(`token${token}unique_id${id}version1.5.11${b}${mm}`)
        body = (ytbody).replace(/time=\d+&sign=\w+/g,`time=${b}&sign=${sign}`)
        let url = {
            url : `https://www.tupinh.com/index.php/api/user/signin`,
            headers : JSON.parse(ythd),
            body : body,
        }
...
//领分红
function ytlfh(timeout = 0) {
    return new Promise((resolve) => {
        sign =MD5_Encrypt(`token${token}unique_id${id}version1.5.11${b}${mm}`)
        body = (ytbody).replace(/time=\d+&sign=\w+/g,`time=${b}&sign=${sign}`)
        let url = {
            url : `https://www.tupinh.com/index.php/api/finance/saveContributionBonus`,
            headers : JSON.parse(ythd),
            body : body,
        }
        $.post(url, async (err, resp, data) => {
...
//结算分红
function ytjsfh(timeout = 0) {
    return new Promise((resolve) => {
        sign =MD5_Encrypt(`token${token}unique_id${id}version1.5.11${b}${mm}`)
        body = (ytbody).replace(/time=\d+&sign=\w+/g,`time=${b}&sign=${sign}`)
        let url = {
            url : `https://www.tupinh.com/index.php/api/finance/contributionBonus2Money`,
            headers : JSON.parse(ythd),
            body : body,
        }
        $.post(url, async (err, resp, data) => {
