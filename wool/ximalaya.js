/**
 [task_local]
 12 8 * * * https://raw.githubusercontent.com/misyili/jd-own/master/wool/ximalaya.js, tag=喜马拉雅签到, enabled=true
...
;(exec = async () => {
    if (!XIMALAYA_COOKIE) {
        console.log("未配置喜马拉雅Cookie， 不执行脚本！")
        return
    }
    let cookies = XIMALAYA_COOKIE.split("|||");
    for (let i=0;i<cookies.length;i++) {
        VAL_signcookie = cookies[i];
        noticeMessage += '\n ---------- 账号： ' + (i+1) + ' ---------------\n';
    ...
    return new Promise((resolve, reject) => {
        ...
        $.post(url, (error, response, data) => {
            try {
                ...
                notify.sendNotify(`❌ ${$.name} signapp - 签到失败: ${e}`)
                resolve()
            }
        })
    })
